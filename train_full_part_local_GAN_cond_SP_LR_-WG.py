...
class Runner(object):
    ...
    def train_one_epoch(self, cur_e = 0):
        ...
        for i_b, sb in enumerate(self.train_dl):
            ...
            if self.i_batch_tot % opt.disp_freq == 0:
                ...
                self.writer.add_image('train/guide-gt-blur-res-local_gt-local_res', torch.cat([gd[:opt.disp_img_cnt], gt[:opt.disp_img_cnt], bl[:opt.disp_img_cnt], res[:opt.disp_img_cnt], local_real[:opt.disp_img_cnt], local_fake[:opt.disp_img_cnt]], 2), self.i_batch_tot)
    ...
    def test(self, cur_e = 0):
        ...
        for i_b, sb in enumerate(self.test_dl):
            ...
            if i_b % opt.print_freq == 0:
                ...
                self.writer.add_image('test/guide-gt-blur-res', torch.cat([gd[:opt.disp_img_cnt], gt[:opt.disp_img_cnt], bl[:opt.disp_img_cnt], res[:opt.disp_img_cnt]], 2), self.i_batch_tot)
    ...
    def load_checkpoint(self):
        ...
        if opt.load_checkpoint:
            ...
            model_state_dict = {k: v for k, v in model_state_dict.items() if 'warpNet' not in k}
            del model_state_dict['recNet.encoder.0.weight']
            cur_model_state_dict = self.G.state_dict()
            cur_model_state_dict.update(model_state_dict)
            # pdb.set_trace()
            self.G.load_state_dict(cur_model_state_dict)
            if 'model_GD' in ckpt:
