...
from custom_criterions import MaskedMSELoss, TVLoss, SymLoss, FullSymLoss, BilinearFullSymLoss, MaskedBilinearFullSymLoss, Face2FaceLoss
import random
from os import path
...
class Runner(object):
    ...
    def train_one_epoch(self, cur_e = 0):
        ...
        for i_b, sb in enumerate(self.train_dl):
            ...
            self.ms['f2f'].add(f2f_l.item())
            self.ms['tot'].add(tot_l.item())

            self.i_batch_tot += 1

            if i_b % opt.print_freq == 0:
                print ('[Train]: %s [%d/%d] (%d/%d)\tPt Loss=%.12f\tTV Loss=%.12f\tSym Loss=%.12f\tF2F Loss=%.12f\tTot Loss=%.12f' % (
            ...
            if self.i_batch_tot % opt.disp_freq == 0:
                ...
                self.writer.add_scalar('train/f2f_loss', self.ms['f2f'].mean, self.i_batch_tot)


        print ('*' * 30)
        print ('[Train]: %s [%d/%d]\tPt Loss=%.12f\tTV Loss=%.12f\tSym Loss=%.12f\tF2F Loss=%.12f\tTot Loss=%.12f' % (
                    time.strftime("%m-%d %H:%M:%S", time.localtime()),
                    cur_e,
                    opt.max_epoch,
                    self.ms['pt'].mean,
    ...
    def test(self, cur_e = 0):
        ...
        for i_b, sb in enumerate(self.test_dl):
            with torch.no_grad():
                ...
                sym_l = torch.Tensor([0]).to(device)
                if opt.test_sym_dir and opt.test_mask_dir:
                    sym_gt = sb['sym_l'].to(device)
                    sym_gd = sb['sym_r'].to(device)
                    mask = sb['mask'].to(device)
                    # sym_l = opt.sym_l_w * self.sym_crit(grid, sym_gd)
            ...
            if i_b % opt.print_freq == 0:
                print ('[Test]: %s [%d/%d] (%d/%d)\tPt Loss=%.12f\tTV Loss=%.12f\tSym Loss=%.12f\tF2F Loss=%.12f\tTot Loss=%.12f' % (
        ...
        print ('=' * 30)
        print ('[Test]: %s [%d/%d]\tPt Loss=%.12f\tTV Loss=%.12f\tSym Loss=%.12f\tF2F Loss=%.12f\tTot Loss=%.12f' % (
                    time.strftime("%m-%d %H:%M:%S", time.localtime()),
                    cur_e,
                    opt.max_epoch,
                    self.ms['pt'].mean,
    ...
    def prepare_losses(self):
        ...
        if opt.train_sym_dir:
            ...
            # self.sym_crit = BilinearFullSymLoss(opt.C)
            self.sym_crit = MaskedBilinearFullSymLoss(opt.C)

        if opt.face_masks_dir:
            self.f2f_crit = Face2FaceLoss()
