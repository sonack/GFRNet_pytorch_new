...
from custom_criterions import MaskedMSELoss, SymLoss, TVLoss, VggFaceLoss
from custom_utils import (Meter, create_orig_xy_map, make_dir,
                          make_face_region_batch, make_parts_region_batch,
                          weight_init)
...
class Runner(object):
    ...
    def test_guided_filter(self):
        ...
        w_gd_dir = path.join(opt.str_dir, 'warp_guide')
        make_dir(w_gd_dir)
        vary_dir = path.join(opt.str_dir, 'varying')
        for i_b, sb in tqdm(enumerate(self.load_dl)):
            with torch.no_grad():
                ...
                w_gd_n_fn = [path.join(w_gd_dir, f_name) for f_name in fn]
                n_fn = [path.join(opt.str_dir, f_name) for f_name in fn]
                
                n_vary_dir = [path.join(vary_dir, f_name) for f_name in fn]
                for nvd in n_vary_dir:
                    make_dir(nvd)
                
                w_gd, grid, res = self.G(bl, gd)
                bg_res = res * (1 - mask.unsqueeze(1))
                bs = gd.size(0)
                # x, y: (N, C, H, W)
                # r 越大 越接近warp_guide
                # r 越小 越接近restored img
                for r in range(0,52):
                    # r = 1
                    # eps 越大 越平滑
                    # eps 越小 越锐利
                    eps = 1e-8
                    if r > 0 and r < 51:
                        res_gf = GuidedFilter(r, eps)(w_gd, res)
                    else:
                        if r == 0:
                            res_gf = res
                        elif r == 51:
                            res_gf = w_gd
                    res_gf = bg_res + res_gf
                    for b_id in range(bs):
                        save_image(res_gf[b_id].view(1, 3, opt.img_size, opt.img_size), path.join(n_vary_dir[b_id], '%02d.png' % r), padding = 0)

                
                for b_id in tqdm(range(bs)):
                    save_image(res[b_id].view(1, 3, opt.img_size, opt.img_size), n_fn[b_id], padding = 0)
                    save_image(w_gd[b_id].view(1, 3, opt.img_size, opt.img_size), w_gd_n_fn[b_id], padding = 0)
    ...
    def prepare_data(self):
        ...
        if opt.load_sbt_dir:
            self.load_dataset = dataset.LoadFaceDataset(opt.load_sbt_dir, load_tsfm_c, opt.test_mask_dir)
